...
{
  // Insert interval
  mModel->insertInterval(TimeRecord(startDate().toUTC(), finishDate().toUTC(), mTimeline->taskId()));
  emit accepted();
  close();
}
...
{
  assert(mTimeline);
  return !mTimeline->hasIntersection(TimeRecord(startDate().toUTC(), finishDate().toUTC(), mTimeline->taskId()));
}
